...
<head>
  ...
  <link rel="icon" type="image/png" href="{{url('public/backend/dist/img/AdminLTELogo.png')}}">
...
<!-- jQuery -->
<script src="{{url('public/backend/plugins/plugins/jquery/jquery.min.js')}}"></script>
<!-- jQuery -->
...
function duplicateEmail(element) {
  var email = $(element).val(); // Ensure jQuery ($) is defined before using it
  $.ajax({
    // AJAX request code...
  });
}
...
<script>
  function duplicateEmail(element) {
    ...
    $.ajax({
      type: "POST",
      url: "{{ url('checkemail') }}", // corrected URL with quotes
      data: {
        email: email,
        _token: "{{ csrf_token() }}"
      },
...
<!-- Bootstrap 4 -->
<script src="{{url('public/backend/plugins/plugins/bootstrap/js/bootstrap.bundle.min.js')}}"></script>
<!-- AdminLTE App -->
<script src="{{url('public/backend/plugins/dist/js/adminlte.min.js')}}"></script>
